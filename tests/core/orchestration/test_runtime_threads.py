...
async def test_runtime_thread_retrying_turn_error_waits_for_later_completion(
    tmp_path: Path,
) -> None:
    @dataclass
    class _HarnessWithRetryingErrorThenCompletion(_HarnessWithBlockingWait):
        capabilities: frozenset[str] = frozenset(
            ["durable_threads", "message_turns", "interrupt", "event_streaming"]
        )
    ...
    harness = _HarnessWithRetryingErrorThenCompletion()
    service = _build_service(tmp_path, harness)
    workspace_root = tmp_path / "workspace"
    workspace_root.mkdir()
    thread = service.create_thread_target("codex", workspace_root)
...
async def test_runtime_thread_progress_stall_timeout_can_replace_hard_wall_clock(
    tmp_path: Path,
) -> None:
    @dataclass
    class _HarnessWithPeriodicProgress(_HarnessWithWait):
