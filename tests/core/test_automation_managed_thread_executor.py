...
def test_managed_thread_auto_decline_dead_letters_and_escalates(tmp_path: Path) -> None:
    ...
    store.enqueue_job(
        _job(
            ...
            policy={
                ...
                "on_failure": {
                    "executor": {"kind": EXECUTOR_PMA_TURN, "lane_id": "pma:default"},
                    "target": {"thread_target_id": thread_id},
                },
            },
            max_attempts=1,
    ...
    escalations = [
        job for job in store.list_jobs(state=JOB_PENDING) if job.job_id != "job-1"
    ]
    assert len(escalations) == 1
    assert escalations[0].executor["kind"] == EXECUTOR_PMA_TURN
