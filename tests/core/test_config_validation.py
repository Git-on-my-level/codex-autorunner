...
class TestValidateAppServerRangeChecks:
    def test_max_handles_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"max_handles": 0}})

    def test_max_handles_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"max_handles": -1}})

    def test_max_handles_positive_ok(self) -> None:
        _validate_app_server_config({"app_server": {"max_handles": 5}})

    def test_idle_ttl_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"idle_ttl_seconds": 0}})

    def test_idle_ttl_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"idle_ttl_seconds": -1}})

    def test_idle_ttl_positive_ok(self) -> None:
        _validate_app_server_config({"app_server": {"idle_ttl_seconds": 300}})

    def test_turn_timeout_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"turn_timeout_seconds": 0}})

    def test_turn_timeout_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"turn_timeout_seconds": -1}})

    def test_turn_timeout_positive_ok(self) -> None:
        _validate_app_server_config({"app_server": {"turn_timeout_seconds": 30}})

    def test_request_timeout_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"request_timeout": 0}})

    def test_request_timeout_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config({"app_server": {"request_timeout": -1}})

    def test_request_timeout_positive_ok(self) -> None:
        _validate_app_server_config({"app_server": {"request_timeout": 30}})

    def test_turn_stall_timeout_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config(
                {"app_server": {"turn_stall_timeout_seconds": 0}}
            )

    def test_turn_stall_timeout_positive_ok(self) -> None:
        _validate_app_server_config({"app_server": {"turn_stall_timeout_seconds": 60}})

    def test_turn_stall_poll_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config(
                {"app_server": {"turn_stall_poll_interval_seconds": 0}}
            )

    def test_turn_stall_poll_positive_ok(self) -> None:
        _validate_app_server_config(
            {"app_server": {"turn_stall_poll_interval_seconds": 2}}
        )

    def test_turn_stall_recovery_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be >= 0 or null"):
            _validate_app_server_config(
                {"app_server": {"turn_stall_recovery_min_interval_seconds": -1}}
            )

    def test_turn_stall_recovery_zero_ok(self) -> None:
        _validate_app_server_config(
            {"app_server": {"turn_stall_recovery_min_interval_seconds": 0}}
        )

    def test_turn_stall_recovery_positive_ok(self) -> None:
        _validate_app_server_config(
            {"app_server": {"turn_stall_recovery_min_interval_seconds": 10}}
        )

    def test_turn_stall_max_attempts_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_app_server_config(
                {"app_server": {"turn_stall_max_recovery_attempts": 0}}
            )

    def test_turn_stall_max_attempts_positive_ok(self) -> None:
        _validate_app_server_config(
            {"app_server": {"turn_stall_max_recovery_attempts": 8}}
        )


class TestValidateOpencodeRangeChecks:
    def test_session_stall_timeout_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config(
                {"opencode": {"session_stall_timeout_seconds": 0}}
            )

    def test_session_stall_timeout_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config(
                {"opencode": {"session_stall_timeout_seconds": -1}}
            )

    def test_session_stall_timeout_positive_ok(self) -> None:
        _validate_opencode_config({"opencode": {"session_stall_timeout_seconds": 30}})

    def test_max_text_chars_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config({"opencode": {"max_text_chars": 0}})

    def test_max_text_chars_negative_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config({"opencode": {"max_text_chars": -1}})

    def test_max_text_chars_positive_ok(self) -> None:
        _validate_opencode_config({"opencode": {"max_text_chars": 1000}})

    def test_max_handles_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config({"opencode": {"max_handles": 0}})

    def test_max_handles_positive_ok(self) -> None:
        _validate_opencode_config({"opencode": {"max_handles": 4}})

    def test_idle_ttl_zero_raises(self) -> None:
        with pytest.raises(ConfigError, match="must be > 0 or null"):
            _validate_opencode_config({"opencode": {"idle_ttl_seconds": 0}})
