...
def test_scope_urn_miss_preserves_workspace_path_for_repo_urn(tmp_path: Path) -> None:
    ...
    checkout.mkdir()
    index = workspace_scope_index_from_snapshots([])
    urn = ScopeRef(kind="repo", id="not-in-topology").to_urn()

    resolution = index.resolve(scope_urn=urn, workspace_path=str(checkout))

    assert resolution is not None
    assert resolution.scope == ScopeRef(kind="repo", id="not-in-topology")
    assert resolution.owner_fields()["workspace_root"] == str(checkout.resolve())
