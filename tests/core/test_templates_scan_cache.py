...
def _redacted(record: TemplateScanRecord) -> TemplateScanRecord:
    return dataclasses.replace(record, evidence=None)
...
def test_scan_cache_read_write(tmp_path: Path) -> None:
    ...
    assert loaded == _redacted(record)
...
def test_scan_multiple_records(tmp_path: Path) -> None:
    ...
    assert loaded1 == _redacted(record1)
    assert loaded2 == _redacted(record2)
    assert loaded3 == _redacted(record3)
...
def test_scan_record_overwrite(tmp_path: Path) -> None:
    ...
    assert loaded is not None
    assert loaded == _redacted(updated)
    assert loaded.decision == "approve"
