...
from codex_autorunner.surfaces.cli import (
    hub_control_plane_client,
    managed_thread_commands,
)
...
def _patch_send_timeout_recovery_env(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        _always_timeout_with_status,
    )
    monkeypatch.setattr(managed_thread_commands, "_request_json", _status_req)
    monkeypatch.setattr(hub_control_plane_client, "request_json", _status_req)
    monkeypatch.setattr(hub_control_plane_client.time, "monotonic", monotonic_fn)
    monkeypatch.setattr(
        hub_control_plane_client.time, "sleep", sleep_fn or (lambda s: None)
    )
...
def test_pma_tail_snapshot_render_lines_use_typed_serializer() -> None:
    snapshot = managed_thread_commands._PmaTailSnapshot.from_dict(
        {
...
def test_managed_thread_status_snapshot_render_lines_include_queue_and_excerpt() -> (
    None
):
    snapshot = managed_thread_commands._ManagedThreadStatusSnapshot.from_dict(
        {
...
def test_managed_thread_status_snapshot_info_level_filters_progress_noise() -> None:
    snapshot = managed_thread_commands._ManagedThreadStatusSnapshot.from_dict(
        {
            "managed_thread_id": "thread-1",
            "thread": {
    ...
    lines = snapshot.render_lines(level=managed_thread_commands._PmaVerbosityLevel.INFO)
...
def test_pma_tail_snapshot_debug_level_keeps_raw_progress_noise() -> None:
    ...
    lines = snapshot.render_lines(
        level=managed_thread_commands._PmaVerbosityLevel.DEBUG
    )
...
def test_managed_thread_send_request_and_response_helpers() -> None:
    request = managed_thread_commands._ManagedThreadSendRequest(
        message="follow up",
    ...
    response = managed_thread_commands._ManagedThreadSendResponse.from_http(
        200,
        {
            "status": "ok",
            "send_state": "accepted",
    ...
    error_response = managed_thread_commands._ManagedThreadSendResponse.from_http(
        409,
        {
            "status": "error",
            "send_state": "rejected",
...
def test_pma_cli_thread_query_commands_use_orchestration_routes(
    ...
    monkeypatch.setattr(managed_thread_commands, "_request_json", _fake_request_json)
...
def test_pma_cli_thread_cancel_and_clear_queue_commands(
    ...
    monkeypatch.setattr(
        managed_thread_commands,
        "_request_json_with_status",
        _fake_request_json_with_status,
    )
    monkeypatch.setattr(managed_thread_commands, "_request_json", _fake_request_json)
...
def test_pma_cli_thread_send_uses_extended_post_timeout(
    ...
    assert (
        captured["timeout"]
        == hub_control_plane_client.MANAGED_THREAD_SEND_REQUEST_TIMEOUT_SECONDS
    )
...
def test_pma_cli_thread_send_retries_timeout_recovery_until_status_catches_up(
    ...
    assert "recovered delivery from thread status" in result.stdout
    assert sleep_calls == [
        hub_control_plane_client.MANAGED_THREAD_SEND_TIMEOUT_RECOVERY_POLL_SECONDS
    ]
...
def test_pma_cli_thread_create_rejects_backend_id_option() -> None:
    sig = inspect.signature(managed_thread_commands.managed_thread_spawn)
...
def test_pma_cli_thread_archive_reports_unreachable_host_port(
    ...
    monkeypatch.setattr(managed_thread_commands, "_request_json", _raise_connect_error)
...
def test_pma_cli_thread_archive_reports_base_path_mismatch(
    ...
    monkeypatch.setattr(managed_thread_commands, "_request_json", _raise_http_status)
...
def test_pma_cli_thread_archive_preserves_not_found_detail(
    ...
    monkeypatch.setattr(
        managed_thread_commands, "_request_json", _raise_thread_not_found
    )
