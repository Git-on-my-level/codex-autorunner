...
def test_hub_repo_enricher_sets_repo_state_ttl_at_write_time(
    tmp_path: Path,
    monkeypatch,
) -> None:
    hub_root = tmp_path / "hub"
    ...
    enricher = HubRepoEnricher(context, _MountManager())  # type: ignore[arg-type]
    calls = {"compute": 0}
    monotonic_values = iter((10.0, 12.5, 12.6))

    def fake_compute(*_args, **_kwargs) -> dict[str, object]:
        calls["compute"] += 1
        return {"status": f"payload-{calls['compute']}"}

    monkeypatch.setattr(
        "codex_autorunner.surfaces.web.routes.hub_repo_routes.services.time.monotonic",
        lambda: next(monotonic_values),
    )
    monkeypatch.setattr(enricher, "_compute_repo_state_payload", fake_compute)

    first = enricher._repo_state_payload(snapshot, stale_threshold_seconds=None)
    second = enricher._repo_state_payload(snapshot, stale_threshold_seconds=None)

    assert first == {"status": "payload-1"}
    assert second == {"status": "payload-1"}
...
def test_gather_hub_message_snapshot_sets_ttl_at_write_time(
    tmp_path: Path,
    monkeypatch,
) -> None:
    hub_root = tmp_path / "hub"
    hub_root.mkdir(parents=True, exist_ok=True)
    calls = {"list_repos": 0}
    monotonic_values = iter((20.0, 22.5, 22.6))
    ...
    monkeypatch.setattr(
        "codex_autorunner.surfaces.web.services.hub_gather.time.monotonic",
        lambda: next(monotonic_values),
    )
