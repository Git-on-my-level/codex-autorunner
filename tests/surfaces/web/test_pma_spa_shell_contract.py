"""Contract: derived SvelteKit +page URLs get PMA HTML shell (or legacy redirect)."""

from __future__ import annotations

from pathlib import Path

from scripts.check_pma_hub_spa_shell import collect_probe_paths, routes_dir, run_checks
...
def test_collect_probe_paths_matches_repo_routes() -> None:
    ...
    assert "/repos/probe-repo/memory" in paths
    assert "/hub" in paths
    assert "/worktrees" in paths
    assert "/contextspace/probe-workspace" in paths


def test_pma_spa_shell_checker_passes_on_repo_tree() -> None:
    root = Path(__file__).resolve().parents[3]
