...
async def test_ticket_runner_requires_commit_before_advancing_done_ticket(
    ...
    assert first.state["commit"]["barrier_epoch"].startswith("commit-barrier:")
    assert first.state["commit"]["resolution_state"] == "pending"
    assert first.state["commit"]["first_seen_at"]
    assert (
...
@pytest.mark.asyncio
async def test_ticket_runner_does_not_retry_exhausted_commit_barrier(
    tmp_path: Path,
) -> None:
    workspace_root = tmp_path
    _init_git_repo(workspace_root)
    ...
    ticket_dir.mkdir(parents=True, exist_ok=True)
    ticket_path = ticket_dir / "TICKET-001.md"
    _write_ticket(ticket_path, done=True)
    (workspace_root / "work.txt").write_text("dirty\n", encoding="utf-8")

    pool = FakeAgentPool(lambda req: pytest.fail("agent should not be called"))
    runner = TicketRunner(
        workspace_root=workspace_root,
        run_id="run-1",
        config=TicketRunConfig(
    ...
    assert result.status == "paused"
    assert result.reason == "Commit barrier exhausted. Manual commit required."
    assert result.state.get("commit", {}).get("barrier_epoch") == "commit-barrier:test"
