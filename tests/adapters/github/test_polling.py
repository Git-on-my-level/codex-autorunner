...
import codex_autorunner.adapters.github.polling as github_polling
import codex_autorunner.core.scm_polling_watches as scm_polling_watches
from codex_autorunner.adapters.github.polling import (
    GitHubPollingConfig,
...
from codex_autorunner.adapters.github.service import GitHubError, RepoInfo
...
class _PostOpenBoostDueScenario:
    name: str
...
_POST_OPEN_BOOST_DUE_SCENARIOS = [
    _PostOpenBoostDueScenario(
        name="active-window-keeps-boost",
        initial_snapshot={
            "head_sha": "oldsha",
            "pr_state": "open",
    ...
    _PostOpenBoostDueScenario(
        name="baseline-pending-initializes-boost",
        initial_snapshot={"baseline_pending": True},
    ),
]
...
def test_process_due_watches_applies_post_open_boost_scenarios(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    scenario: _PostOpenBoostDueScenario,
) -> None:
    binding = PrBindingStore(tmp_path).upsert_binding(
        provider="github",
        repo_slug="acme/widgets",
