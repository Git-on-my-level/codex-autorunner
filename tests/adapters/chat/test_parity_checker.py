...
from codex_autorunner.adapters.chat import parity_checker
from codex_autorunner.adapters.chat.action_ux_contract import (
    CHAT_ACTION_UX_CONTRACT,
)
from codex_autorunner.adapters.chat.command_contract import (
    COMMAND_CONTRACT,
    CommandContractEntry,
)
from codex_autorunner.adapters.chat.parity_checker import run_parity_checks
...
def _write_fixture_repo(
    ...
    *,
    ...
    _write_text(
        root / "src/codex_autorunner/adapters/discord/service.py",
        discord_service,
    )
    _write_text(
        root / "src/codex_autorunner/adapters/telegram/trigger_mode.py",
        telegram_trigger_mode,
    )
    _write_text(
        root / "src/codex_autorunner/adapters/telegram/handlers/messages.py",
        telegram_messages,
    )
...
def _build_discord_service_fixture(
    *,
    ...
    import_line = (
        "from ...adapters.chat.command_ingress import canonicalize_command_ingress\n"
        if include_canonicalize_usage
        else ""
    )
    collaboration_policy_import = (
        "from ...adapters.chat.collaboration_policy import evaluate_collaboration_policy\n"
...
"""
    ...
    return (
        "from ...adapters.chat.turn_policy import PlainTextTurnContext, should_trigger_plain_text_turn\n"
        + collaboration_policy_import
        + import_line
        + """
