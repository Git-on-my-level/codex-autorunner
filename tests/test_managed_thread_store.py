...
def test_create_list_get_thread(tmp_path: Path) -> None:
    ...
    assert store.path == default_managed_threads_db_path(hub_root)
...
def test_prepare_runs_legacy_backfill_before_marking_prepared(
    ...
    monkeypatch.setattr(
        "codex_autorunner.core.managed_thread_store_bootstrap.ensure_legacy_orchestration_backfill",
        fake_backfill,
    )

    ManagedThreadStore(tmp_path / "hub")
...
def test_create_thread_enriches_head_branch_metadata(
    ...
    workspace_root.mkdir()
    monkeypatch.setattr(
        "codex_autorunner.core.managed_thread_store_rows.workspace_head_branch",
        lambda repo_root: "feature/pr-binding",
    )

    created = ManagedThreadStore(hub_root).create_thread("codex", workspace_root)
...
def test_create_thread_rejects_mixed_canonical_and_legacy_owner(
    ...
    with pytest.raises(ValueError, match="scope cannot be combined"):
        ManagedThreadStore(tmp_path / "hub").create_thread(
            "codex",
            workspace_root,
            scope=ScopeRef(kind="repo", id="repo-123"),
            repo_id="repo-456",
...
def test_connect_readonly_skips_bootstrap_initialize(
    ...
    monkeypatch.setattr(
        "codex_autorunner.core.managed_thread_store.ManagedThreadStoreBootstrap.initialize",
        _record_initialize,
    )
...
def test_connect_readonly_does_not_prepare_shared_state_on_read(
    ...
    monkeypatch.setattr(
        "codex_autorunner.core.managed_thread_store_bootstrap.prepare_orchestration_sqlite",
        _fail_prepare,
    )
...
def test_create_turn_recovers_stale_running_execution_before_admission(
    ...
    assert store.mark_turn_finished(stale_running_turn["managed_turn_id"], status="ok")
    active_turn = store.create_turn(thread["managed_thread_id"], prompt="second")
    assert store.mark_turn_interrupted(active_turn["managed_turn_id"])

    # Simulate corrupted state: an older turn is incorrectly left as running.
...
def test_claim_next_queued_turn_recovers_stale_running_execution(
    ...
    queued_turn = store.create_turn(
        ...
        request_kind="review",
        busy_policy="queue",
        queue_payload={"request": {"message_text": "third queued", "kind": "review"}},
    )
    assert queued_turn["status"] == "queued"
    assert store.mark_turn_interrupted(active_turn["managed_turn_id"])
...
def test_concurrent_create_turn_admission_is_atomic(tmp_path: Path) -> None:
    hub_root = tmp_path / "hub"
    store_a = ManagedThreadStore(hub_root)
    store_b = ManagedThreadStore(hub_root)
    thread = store_a.create_thread("codex", tmp_path / "workspace")
    barrier = threading.Barrier(2)

    def _attempt_turn(store: ManagedThreadStore, prompt: str) -> tuple[str, str | None]:
...
def test_archived_thread_terminalizes_live_running_turn(tmp_path: Path) -> None:
    ...
    assert running_after["error"] == "thread_archived"
    ...
    assert queued_after is not None
    assert queued_after["status"] == "interrupted"
    assert queued_after["error"] == "thread_archived"
...
def test_schema_creation_is_idempotent(tmp_path: Path) -> None:
    hub_root = tmp_path / "hub"
    first = ManagedThreadStore(hub_root)
    second = ManagedThreadStore(hub_root)
...
def test_thread_record_normalizes_legacy_status_aliases(tmp_path: Path) -> None:
    ...
    assert row is not None
    record = ManagedThreadRecord.from_orchestration_row(row).to_dict()
...
def test_thread_record_migrates_alias_only_status_fields_to_canonical_schema() -> None:
    record = ManagedThreadRecord.from_store_mapping(
        {
...
def test_concurrentish_writes_smoke(tmp_path: Path) -> None:
    ...
    def _write_one(store: ManagedThreadStore, name: str) -> str:
    ...
    assert managed_threads_db_lock_path(store_a.path).exists()
