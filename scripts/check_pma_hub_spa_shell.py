...
``pma_frontend/src/routes`` (same rules as SvelteKit dynamic segments) and
asserts each probe succeeds with either:
  - 200 + PMA Hub HTML markers, or
  - 307/308 (legacy redirects for /worktrees and /contextspace).

Usage:
  python scripts/check_pma_hub_spa_shell.py
...
def routes_dir(repo_root: Path) -> Path:
    return repo_root / "src" / "codex_autorunner" / "pma_frontend" / "src" / "routes"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for path in paths:
            ...
            if response.status_code != 200:
                ...
                continue
            body = response.text
            missing = [m for m in _PMA_MARKERS if m not in body]
            if missing:
                errors.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        print("PMA Hub SPA shell check failed:", file=sys.stderr)
        for line in errors:
            print(f"  - {line}", file=sys.stderr)
        print(
            "\nFix: add matching @app.get routes that return _pma_index_response() "
            "(see surfaces/web/app.py, comment “PMA Hub SPA shell”).",
            file=sys.stderr,
        )
        return 1
    print(f"PMA Hub SPA shell check OK ({repo_root}) — {len(paths)} paths")
    return 0
