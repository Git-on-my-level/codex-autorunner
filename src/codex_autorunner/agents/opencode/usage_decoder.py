...
def extract_usage_details(usage: dict[str, Any]) -> dict[str, int]:
    ...
    if total is not None:
        details["totalTokens"] = total

    return details


def extract_context_window(payload: Any, usage: dict[str, Any]) -> Optional[int]:
    """Extract context window from payload."""
    if not isinstance(payload, dict):
        return None

    info = payload.get("info")
    if isinstance(info, dict):
        context = info.get("contextWindow") or info.get("context_window")
        if isinstance(context, int):
            return context

        model = info.get("model")
        if isinstance(model, dict):
            context = model.get("contextWindow") or model.get("context_window")
            if isinstance(context, int):
                return context
