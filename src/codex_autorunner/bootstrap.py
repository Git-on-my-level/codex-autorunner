...
def _write_doc_if_changed(path: Path, content: str) -> None:
    try:
...
def ensure_pma_docs(hub_root: Path, force: bool = False) -> None:
    ...
    for filename, content_fn in seeds:
        ...
        if filename in PMA_ALWAYS_REFRESH_DOCS:
            _write_doc_if_changed(canonical_path, content)
...
- Use CAR-native artifacts (tickets, ticket_flow, dispatch, artifact delivery, PMA file uploads).
...
- Check destination before runtime-sensitive work:
...
## Destinations (local/docker runtime)
