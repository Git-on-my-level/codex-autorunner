# Web Hub Frontend — Design System

This doc captures the visual and interaction language for the SvelteKit frontend
under `src/codex_autorunner/web_frontend/`. It exists so future agents can ship
...
```bash
cd src/codex_autorunner/web_frontend && pnpm run build
```

This regenerates `web_static/`. The hub serves these directly; no extra
deploy step.
