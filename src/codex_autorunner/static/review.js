...
async function loadReviewStatus() {
    try {
        ...
        setLink(bundleLink, {
            href: review.scratchpad_bundle_path ? `/api/review/artifact?kind=scratchpad_bundle` : null,
            text: "Scratchpad",
...
async function startReview() {
    try {
        ...
        const payload = {
            ...
            max_wallclock_seconds: timeoutEl?.value ? parseInt(timeoutEl.value, 10) || null : null,
        };

        const data = await api("/api/review/start", { method: "POST", body: payload });
        ...
        flash(err.message || "Failed to start review");
    }
}

async function stopReview() {
    try {
        const data = await api("/api/review/stop", { method: "POST" });
        if (data.status === "ok" || data.review) {
        ...
        flash(err.message || "Failed to stop review");
    }
}

async function resetReview() {
    try {
        const data = await api("/api/review/reset", { method: "POST" });
        if (data.status === "ok" || data.review) {
        ...
        flash(err.message || "Failed to reset review");
    }
}

export function initReview() {
    $("review-start")?.addEventListener("click", startReview);
    $("review-stop")?.addEventListener("click", stopReview);
    $("review-reset")?.addEventListener("click", resetReview);
