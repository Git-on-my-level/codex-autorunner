...
let pmaDeliveryTargets = [];
let pmaManagedThreads = [];
let selectedManagedThreadId = null;
let selectedManagedThread = null;
...
function renderPMADeliveryTargets() {
    const elements = getElements();
...
async function loadPMADeliveryTargets(options = {}) {
    ...
    try {
        ...
        pmaDeliveryTargets = items
            ...
            return {
                ...
                last_delivery_turn_id: typeof item?.last_delivery_turn_id === "string" && item.last_delivery_turn_id
...
async function addPMADeliveryTarget() {
    const elements = getElements();
    ...
    try {
        ...
        flash(err.message || "Failed to add target", "error");
    }
}
async function removePMADeliveryTarget(targetKey) {
    if (!targetKey)
        return;
    try {
        ...
        flash(err.message || "Failed to remove target", "error");
    }
}
async function clearPMADeliveryTargets() {
    ...
    try {
        ...
        flash(err.message || "Failed to clear targets", "error");
    }
}
function renderPMAManagedThreadDetail() {
    const elements = getElements();
    if (!elements.managedThreadDetail)
        return;
...
function renderPMAManagedThreadsList() {
    const elements = getElements();
...
async function inspectPMAManagedThread(managedThreadId, options = {}) {
    const { silent = false } = options;
    if (!managedThreadId)
        return;
    try {
        const payload = (await api(`/hub/pma/threads/${encodeURIComponent(managedThreadId)}`, { method: "GET" }));
        ...
        if (!silent) {
            flash(err.message || "Failed to inspect managed thread", "error");
        }
    }
}
async function loadPMAManagedThreads(options = {}) {
    ...
    try {
        ...
        pmaManagedThreads = threads.filter((thread) => !!thread && typeof thread.managed_thread_id === "string" && !!thread.managed_thread_id);
        renderPMAManagedThreadsList();
        if (preserveSelection && selectedManagedThreadId) {
            const exists = pmaManagedThreads.some((thread) => thread.managed_thread_id === selectedManagedThreadId);
            if (exists) {
...
async function archiveSelectedPMAManagedThread() {
    if (!selectedManagedThreadId) {
        flash("Select a managed thread first", "error");
        return;
    }
    try {
        const payload = (await api(`/hub/pma/threads/${encodeURIComponent(selectedManagedThreadId)}/archive`, { method: "POST" }));
        ...
        flash(err.message || "Failed to archive managed thread", "error");
    }
}
async function resumeSelectedPMAManagedThread() {
    const elements = getElements();
    if (!selectedManagedThreadId) {
        flash("Select a managed thread first", "error");
        return;
    ...
    try {
        const payload = (await api(`/hub/pma/threads/${encodeURIComponent(selectedManagedThreadId)}/resume`, {
            method: "POST",
        ...
        flash(err.message || "Failed to resume managed thread", "error");
    }
}
...
function getElements() {
    return {
        ...
        targetsRefInput: document.getElementById("pma-target-ref"),
        targetsAddBtn: document.getElementById("pma-targets-add"),
        targetsClearBtn: document.getElementById("pma-targets-clear"),
        targetsList: document.getElementById("pma-targets-list"),
        managedThreadsRefreshBtn: document.getElementById("pma-managed-threads-refresh"),
        ...
        managedThreadBackendInput: document.getElementById("pma-managed-thread-backend-id"),
        managedThreadArchiveBtn: document.getElementById("pma-managed-thread-archive"),
        managedThreadResumeBtn: document.getElementById("pma-managed-thread-resume"),
    };
}
...
function attachHandlers() {
    ...
    if (elements.managedThreadsList) {
        elements.managedThreadsList.addEventListener("click", (event) => {
            const target = event.target;
            const threadRow = target?.closest?.("[data-managed-thread-id]");
