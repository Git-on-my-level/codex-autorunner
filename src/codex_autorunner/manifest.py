...
def load_manifest_with_issues(
    ...
    if _legacy_ws_section in data:
        raise ManifestError(
            f"{_legacy_ws_section} is no longer supported; remove this section from the hub manifest"
        )
    ...
    if isinstance(agents_block, dict) and _legacy_agent_runtime in agents_block:
        raise ManifestError(
            f"manifest agents.{_legacy_agent_runtime} is no longer supported; remove this section"
        )
