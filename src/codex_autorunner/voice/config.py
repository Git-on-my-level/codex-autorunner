...
class VoiceConfig:
    ...
    def from_raw(
        ...
        """
        Build a normalized VoiceConfig from config.yml voice settings.
        ...
        # Auto-enable voice if the selected remote provider already has credentials.
        if not merged.get("enabled"):
            ...
            if isinstance(api_key_env, str) and api_key_env and env.get(api_key_env):
                merged["enabled"] = True
        merged["provider"] = _normalize_provider_alias(merged.get("provider"))
        ...
        push_to_talk = PushToTalkConfig(
            max_ms=int(pt.get("max_ms", 15_000)),
            silence_auto_stop_ms=int(pt.get("silence_auto_stop_ms", 1_200)),
            min_hold_ms=int(pt.get("min_hold_ms", 150)),
        )
