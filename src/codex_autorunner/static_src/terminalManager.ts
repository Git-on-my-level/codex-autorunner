...
import {
  getSavedSessionId as getSessionId,
  setSavedSessionId as setSessionId,
  clearSavedSessionId as clearSessionId,
  markSessionActive as sessionMarkActive,
  buildConnectQuery,
  createTerminalSocket,
  teardownSocket as sessionTeardownSocket,
  SocketHeartbeat,
  ReconnectScheduler,
...
import {
  createReplayState,
  resetReplayState,
  initReplayForConnect,
  bufferReplayChunk,
  handleReplayEnd,
  consumeLiveReset,
} from "./terminalReplay.js";
import type { ReplayState } from "./terminalReplay.js";
import type { XtermTerminal, XtermFitAddon } from "./terminalTypes.js";
import {
  createTranscriptState,
  resetTranscript,
  restoreTranscript,
  hydrateTerminalFromTranscript,
  appendTranscriptChunk,
  isAltBufferActive,
} from "./terminalTranscript.js";
import type { TranscriptState } from "./terminalTranscript.js";
import {
  TEXT_INPUT_STORAGE_KEYS,
  createTextInputState,
  readBoolFromStorage,
  safeFocus,
  captureTextInputSelection,
  updateTextInputSendUi,
  persistTextInputDraft,
  restoreTextInputDraft,
  loadPendingTextInput,
  sendPendingTextInputChunk,
  handleTextInputAck,
  sendFromTextarea,
  setTextInputEnabled,
  updateComposerSticky,
  registerTextInputHook,
  hasTextInputHookFired,
  markTextInputHookFired,
  migrateTextInputHookSession,
  handleImageFiles,
} from "./terminalTextInput.js";
import type { TextInputState, TextInputDeps } from "./terminalTextInput.js";
import {
  createMobileState,
  ...
  setMobileViewActive,
  scheduleMobileViewRender,
  initMobileControls,
  installWheelScroll,
  installTouchScroll,
} from "./terminalMobile.js";
import type { MobileState, MobileDeps } from "./terminalMobile.js";
import {
  createVoiceState,
  initTerminalVoice,
  handleVoiceHotkeyDown,
  handleVoiceHotkeyUp,
} from "./terminalVoice.js";
import type { VoiceState } from "./terminalVoice.js";
...
export class TerminalManager {
  ...
  reconnect: ReconnectScheduler = new ReconnectScheduler();
  heartbeat: SocketHeartbeat = new SocketHeartbeat(TERMINAL_DEBUG, (msg, details) => this._logTerminalDebug(msg, details));
  ...
  transcript: TranscriptState = createTranscriptState();
  replayState: ReplayState = createReplayState();
  textInput: TextInputState = createTextInputState();
  mobile: MobileState = createMobileState();
  voice: VoiceState = createVoiceState();
  ...
  _logTerminalDebug(message: string, details: unknown = null) {
  ...
  _logBufferSnapshot(reason: string) {
  ...
  _setSavedSessionId(sessionId: string) {
  ...
  _setCurrentSessionId(sessionId: string | null) {
  ...
  _buildCarContextHook() {
    return {
      ...
      apply: ({ text }: { text: string }) => {
        ...
        if (hasTextInputHookFired(CAR_CONTEXT_HOOK_ID, this.currentSessionId, this._getRepoStorageKey())) return null;
  ...
  _getTextInputDeps(): TextInputDeps {
    return {
  ...
  _setMobileViewActive(active: boolean) {
  ...
  _updateButtons(connected: boolean) {
  ...
  _updateTextInputConnected(_connected: boolean): void {
    if (this.textInput.textInputTextareaEl) this.textInput.textInputTextareaEl.disabled = false;
    updateTextInputSendUi(this.textInput, this.socket, this.sessionNotFound);
  }
  ...
  _handleReplayEnd() {
    const flush = handleReplayEnd(
      this.replayState,
      this.transcript.resetForConnect,
      this.transcript.altScrollbackLines.length
    );
    if (!flush) return;
  ...
  _initTextInputPanel() {
    this.textInput.terminalSectionEl = document.getElementById("terminal") as HTMLElement | null;
    this.textInput.textInputToggleBtn = document.getElementById("terminal-text-input-toggle") as HTMLButtonElement | null;
    this.textInput.textInputPanelEl = document.getElementById("terminal-text-input") as HTMLElement | null;
    this.textInput.textInputTextareaEl = document.getElementById("terminal-textarea") as HTMLTextAreaElement | null;
    this.textInput.textInputSendBtn = document.getElementById("terminal-text-send") as HTMLButtonElement | null;
    this.textInput.textInputImageBtn = document.getElementById("terminal-text-image") as HTMLButtonElement | null;
    this.textInput.textInputImageInputEl = document.getElementById("terminal-text-image-input") as HTMLInputElement | null;

    if (
      !this.textInput.terminalSectionEl ||
    ...
    this.textInput.textInputEnabled = readBoolFromStorage(
    ...
    const triggerSend = async () => {
      ...
      const now = Date.now();
      if (now - this.mobile.lastSendTapAt < 300) return;
    ...
    if (this.textInput.textInputImageBtn && this.textInput.textInputImageInputEl) {
      this.textInput.terminalSectionEl!.addEventListener("paste", (e) => {
        if (document.activeElement === this.textInput.textInputTextareaEl) return;
        const items = e.clipboardData?.items;
        if (!items || !items.length) return;
        const files: File[] = [];
        for (const item of Array.from(items)) {
  ...
  _handleVoiceHotkeyDown(event: KeyboardEvent) {
    handleVoiceHotkeyDown(this.voice, event, this.socket);
  }

  _handleVoiceHotkeyUp(event: KeyboardEvent) {
