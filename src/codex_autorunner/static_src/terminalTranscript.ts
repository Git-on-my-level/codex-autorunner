...
export interface TranscriptState {
  lines: TranscriptCell[][];
  lineCells: TranscriptCell[];
  cursor: number;
  maxLines: number;
  hydrated: boolean;
  ansiState: TranscriptAnsiState;
  persistTimer: number | null;
  decoder: TextDecoder;
  resetForConnect: boolean;
  altScrollbackLines: string[];
  altSnapshotPlain: string[] | null;
  altSnapshotHtml: string[] | null;
  xtermPalette: string[] | null;
}
...
export function transcriptStorageKey(): string {
...
export function cellsToSegments(cells: TranscriptCell[]): TranscriptCell[] {
  if (!Array.isArray(cells)) return [];
  const segments: TranscriptCell[] = [];
  let current: TranscriptCell | null = null;
...
export function segmentsToCells(segments: unknown): TranscriptCell[] | null {
  if (typeof segments === "string") {
    return Array.from(segments).map((ch) => ({ t: ch, c: "", s: "" }));
  }
  if (!Array.isArray(segments)) return null;
  const cells: TranscriptCell[] = [];
  for (const seg of segments as TranscriptCell[]) {
...
export function cellsToHtml(cells: TranscriptCell[]): string {
  if (!cells.length) return "";
  const segments = cellsToSegments(cells);
  let html = "";
  for (const seg of segments) {
    const text = escapeHtml(seg.t);
    if (!seg.c && !(seg as { s?: string }).s) {
      html += text;
    } else if (seg.c && (seg as { s?: string }).s) {
      html += `<span class="${seg.c}" style="${(seg as { s?: string }).s}">${text}</span>`;
    } else if (seg.c) {
      html += `<span class="${seg.c}">${text}</span>`;
    } else {
      html += `<span style="${(seg as { s?: string }).s}">${text}</span>`;
    }
  }
  return html;
}

export function cellsToPlainText(cells: TranscriptCell[]): string {
...
export function resetTranscript(state: TranscriptState): void {
  state.lines = [];
  state.lineCells = [];
  state.cursor = 0;
...
export function restoreTranscript(state: TranscriptState): void {
  try {
    ...
    if (Array.isArray(parsed?.lines)) {
      state.lines = parsed!
        ...
        .filter(Boolean) as TranscriptCell[][];
    }
...
export function persistTranscript(state: TranscriptState, clear = false): void {
  try {
    const key = transcriptStorageKey();
    if (clear) {
      try {
...
export function persistTranscriptSoon(state: TranscriptState): void {
...
export function getTranscriptLines(state: TranscriptState): TranscriptCell[][] {
...
export interface BufferSnapshot {
  bufferNamespace: XtermTerminal["buffer"];
  buffer: XtermTerminal["buffer"]["active"];
  lines: (
    | { translateToString(trimRight?: boolean): string; getCell(col: number): unknown; length: number; getWidth?(): number; getCode?(): number; getChars?(): string; isBold?(): boolean; isInverse?(): boolean; isInvisible?(): boolean; getFgColorMode?(): number; getBgColorMode?(): number; getFgColor?(): number; getBgColor?(): number }
    | null
    | undefined
  )[];
  cols: number;
...
export function getBufferSnapshot(term: XtermTerminal | null, maxLines: number): BufferSnapshot | null {
  ...
  const lines: BufferSnapshot["lines"] = [];
...
export function snapshotBufferLines(
  ...
  for (const line of snapshot.lines) {
    plain.push(bufferLineToText(line as Parameters<typeof bufferLineToText>[0]));
    html.push(bufferLineToHtml(line as Parameters<typeof bufferLineToHtml>[0], cols));
  }
  return { plain, html };
}

export function findLineOverlap(prevRegion: string[], nextRegion: string[]): number {
...
export function trimAltScrollback(state: TranscriptState): void {
  if (!Array.isArray(state.altScrollbackLines)) return;
...
export function clearAltScrollbackState(state: TranscriptState): void {
  state.altScrollbackLines = [];
  state.altSnapshotPlain = null;
  state.altSnapshotHtml = null;
}

export function updateAltScrollback(
...
export function isAltBufferActive(term: XtermTerminal | null): boolean {
  const bufferNamespace = term?.buffer;
  if (!bufferNamespace?.active || !bufferNamespace?.alternate) return false;
  return bufferNamespace.active === bufferNamespace.alternate;
}

function paletteIndexToCss(state: TranscriptState, index: number): string | null {
...
function rgbNumberToCss(value: number): string | null {
...
function getCellWidth(cell: XtermCell): number {
  if (typeof cell.getWidth === "function") {
    return cell.getWidth();
  }
  if (Number.isInteger((cell as unknown as { width: number }).width)) {
    return (cell as unknown as { width: number }).width;
  }
  return 1;
}

function getCellChars(cell: XtermCell, width: number): string {
  let chars = "";
...
export function bufferLineToHtml(
  line: {
    getCell?(col: number): XtermCell;
    translateToString?(trimRight?: boolean): string;
    length?: number;
  } | null,
  ...
  if (typeof line.getCell !== "function") {
    return escapeHtml(bufferLineToText(line as Parameters<typeof bufferLineToText>[0]));
  }
  let html = "";
  let currentText = "";
  let currentClass = "";
...
function ansiClassName(ansiState: TranscriptAnsiState): string {
  const parts: string[] = [];
  if (ansiState.bold) parts.push("ansi-bold");
  if (ansiState.fg) parts.push(`ansi-fg-${ansiState.fg}`);
  if (ansiState.bg) parts.push(`ansi-bg-${ansiState.bg}`);
  return parts.join(" ");
}

function ansiStyle(ansiState: TranscriptAnsiState): string {
  const styles: string[] = [];
  if (ansiState.fgRgb) styles.push(`color: ${ansiState.fgRgb}`);
  if (ansiState.bgRgb) styles.push(`background-color: ${ansiState.bgRgb}`);
  return styles.join("; ");
}

function ansi256ToRgb(value: number): string | null {
...
function applyAnsiPaletteColor(
...
function pushTranscriptLine(state: TranscriptState, lineCells: TranscriptCell[]): void {
...
export function appendTranscriptChunk(state: TranscriptState, data: string | Uint8Array): void {
  if (!data) return;
  const text =
    typeof data === "string"
      ? data
  ...
  const parseParams = (raw: string): (number | null)[] => {
    if (!raw) return [];
  ...
  const getParam = (params: (number | null)[], index: number, fallback: number): number => {
  ...
  const writeChar = (char: string) => {
    ...
    const cell: TranscriptCell = { t: char, c: ansiState.className, s: ansiState.style || undefined };
    if (ansiState.style) {
...
export function hydrateTerminalFromTranscript(
  state: TranscriptState,
  term: XtermTerminal | null
): void {
  if (!term || state.hydrated) return;
