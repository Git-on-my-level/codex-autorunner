...
import {
  ...
  updateLastActivityFromTimestamp,
  updateActivityFromEvent,
  processStreamDelta,
  processAppServerEvent,
  processStepStarted,
  summarizeReason,
  resetAllStreamState,
  setFlowStartedAt,
  getLastKnownEventAt,
  setLastActivityTime,
  getCurrentReasonFull,
} from "./ticketFlowStream.js";
import {
  renderTickets,
...
async function loadTicketFlow(ctx?: RefreshContext): Promise<void> {
  ...
  try {
    ...
    if (reason) {
      ...
      const runState = (latest?.state || {}) as Record<string, unknown>;
