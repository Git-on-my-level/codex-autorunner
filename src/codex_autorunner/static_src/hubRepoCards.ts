...
export function setCleanupAllInFlight(value: boolean): void {
...
function getHubCleanupAllBtn(): HTMLButtonElement | null {
  return document.getElementById("hub-cleanup-all") as HTMLButtonElement | null;
}
...
function buildMountBadge(repo: HubRepo): string {
  ...
  if (missing) {
    ...
    label = "not mounted";
    className += " pill-warn";
    return `<span class="${className} hub-mount-pill">${escapeHtml(
      label
    )}</span>`;
  } else {
    return "";
...
function updateCleanupAllButton(repos: HubRepo[]): void {
  ...
  if (!btn) return;
  const hasUnboundBaseThreads = repos.some(
    (r) => (r.kind || "base") === "base" && unboundManagedThreadCount(r) > 0
  );
  const hasEligibleWorktree = repos.some(
    (r) =>
      r.kind === "worktree" &&
  ...
  const active =
    hasUnboundBaseThreads || hasEligibleWorktree || hasCompletedFlowCleanupHint;
  const empty = !active;
...
export function renderSummary(repos: HubRepo[], hubData: HubData): void {
...
export function renderReposWithScroll(repos: HubRepo[], hubChannelEntries: HubChannelEntry[], pinnedParentRepoIds: Set<string>): void {
