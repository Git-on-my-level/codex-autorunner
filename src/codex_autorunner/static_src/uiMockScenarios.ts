...
/** Default API bundle for hub + extras; each scenario overrides as needed. */
export interface UiMockScenarioBundle {
  id: string;
  label: string;
  description: string;
  hubData: HubData;
  hubUsage: HubUsageData;
  channels: HubChannelDirectoryResponse;
  hubVersion: { asset_version: string };
  systemUpdateStatus: Record<string, unknown> | null;
  pmaAgents: { agents: unknown[]; default?: string } | null;
}

function baseRepo(
  partial: Partial<HubRepo> & Pick<HubRepo, "id" | "display_name">
): HubRepo {
  const id = partial.id;
  return {
...
const defaultUsage: HubUsageData = {
  codex_home: "~/.codex",
  status: "ok",
  repos: [],
...
const defaultChannels: HubChannelDirectoryResponse = { entries: [] };

type BundlePatch = {
  hubData?: Partial<HubData>;
  hubUsage?: Partial<HubUsageData>;
  channels?: Partial<HubChannelDirectoryResponse>;
  hubVersion?: Partial<{ asset_version: string }>;
  systemUpdateStatus?: Record<string, unknown> | null;
  pmaAgents?: UiMockScenarioBundle["pmaAgents"];
};

function bundle(id: string, label: string, description: string, patch: BundlePatch): UiMockScenarioBundle {
  const hubMerged: HubData = {
    ...defaultHubData,
    ...patch.hubData,
      ...
      repos: [
        {
          id: "acme-app",
          events: 42,
          totals: { total_tokens: 1_200_000, input_tokens: 400_000, cached_input_tokens: 200_000 },
        },
        {
          id: "internal-tools",
          events: 6,
          totals: { total_tokens: 18_000, input_tokens: 9_000, cached_input_tokens: 2_000 },
        },
      ],
...
const pmaHealthyScenario: UiMockScenarioBundle = {
  ...healthyScenario,
  id: "pma-healthy",
  label: "Healthy hub + PMA",
  description:
    "Same as “healthy” but with /hub/pma/agents mocked so the Project Manager toggle works without real PMA config.",
...
const scenarios: Record<string, UiMockScenarioBundle> = {
  ...
  running: bundle(
    "running",
    "Run in progress",
    "One repo in running state; others idle (for status pills and activity column).",
    {
      hubData: {
        repos: [
          baseRepo({
      ...
      hubUsage: {
        ...
        codex_home: "~/.codex",
        repos: [
          { id: "acme-app", events: 3, totals: { total_tokens: 95_000 } },
          { id: "docs-site", events: 1, totals: { total_tokens: 2_000 } },
        ],
      },
    }
  ),

  "worktrees-and-flow": bundle(
    "worktrees-and-flow",
    "Base + worktrees + ticket flow",
    "Base repo with worktree children and an active multi-step ticket flow (progress + labels).",
    {
      hubData: {
        repos: [
          baseRepo({
  ...
  "error-and-missing": bundle(
    "error-and-missing",
    "Init error + missing on disk",
    "Surface warning/error pills, init error text, and missing repo row.",
    {
      hubData: {
        repos: [
          baseRepo({
  ...
  "channel-directory": bundle(
    "channel-directory",
    "Chat channel sidebar rows",
    "Repos stay minimal; channel directory is populated (Discord + PMA-style rows) for the channel column.",
    {
      hubData: {
        repos: [
  ...
  "usage-loading": bundle(
    "usage-loading",
    "Usage still aggregating",
    "Hub works but usage summary returns loading (triggers client retry + cached columns).",
    {
      hubData: {
  ...
  "pma-agents-ok": bundle(
    "pma-agents-ok",
    "PMA enabled (probe + agents list)",
    "Empty hub but PMA agents list present — use with ?view=pma for the PMA shell without real repos.",
    {
      hubData: {
        repos: [],
        agent_workspaces: [],
  ...
  onboarding: bundle(
    ...
    "Empty hub + mocked PMA agents. Pair with ?view=pma and ?carOnboarding=1 to reset the setup walkthrough strip (see walkthrough.ts).",
    {
      hubData: {
        repos: [],
        agent_workspaces: [],
...
export function getUiMockScenarioList(): { id: string; label: string; description: string }[] {
  return UI_MOCK_SCENARIO_ORDER.map((id) => {
    const s = scenarios[id as keyof typeof scenarios];
    return { id: s.id, label: s.label, description: s.description };
  });
}

export function getUiMockScenarioOrDefault(
  id: string | null | undefined
): { scenario: UiMockScenarioBundle; resolvedId: string; fallback: boolean } {
  ...
  if (normalized && !scenarios[normalized]) {
    console.warn(
      `[uiMock] Unknown scenario ${JSON.stringify(id)} — using "empty". Valid: ${Object.keys(scenarios).join(", ")}`
    );
