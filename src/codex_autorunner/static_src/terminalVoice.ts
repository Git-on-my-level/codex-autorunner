...
import type { VoiceController } from "./terminalTypes.js";
...
const VOICE_TRANSCRIPT_DISCLAIMER_TEXT =
  (typeof CONSTANTS !== "undefined" && CONSTANTS.PROMPTS?.VOICE_TRANSCRIPT_DISCLAIMER) ||
...
export interface VoiceState {
  textVoiceBtn: HTMLButtonElement | null;
  voiceBtn: HTMLButtonElement | null;
  voiceStatus: HTMLElement | null;
  voiceController: VoiceController | null;
  voiceKeyActive: boolean;
  mobileVoiceBtn: HTMLButtonElement | null;
  mobileVoiceController: VoiceController | null;
  textVoiceController: VoiceController | null;
}

export interface VoiceDeps {
  getSocket(): WebSocket | null;
  getTerm(): { focus?(): void } | null;
  isTouchDevice(): boolean;
  markSessionActive(): void;
  getTextInputState(): TextInputState;
  getTextInputDeps(): TextInputDeps;
  setTextInputEnabled(state: TextInputState, enabled: boolean, isTouchDevice: boolean, deps: TextInputDeps, options?: { focus?: boolean; focusTextarea?: boolean }): void;
...
export function appendVoiceTranscriptDisclaimer(text: string | null | undefined): string {
...
export function insertTranscriptIntoTextInput(
  _voiceState: VoiceState,
  text: string,
  textInputState: TextInputState,
  isTouchDevice: boolean,
  textInputDeps: TextInputDeps
): boolean {
  if (!text) return false;
  if (!textInputState.textInputTextareaEl) return false;
...
export function matchesVoiceHotkey(event: KeyboardEvent): boolean {
  return event.key && event.key.toLowerCase() === "v" && event.altKey;
}

export function handleVoiceHotkeyDown(
  voiceState: VoiceState,
  event: KeyboardEvent,
  socket: WebSocket | null
): void {
  if (!voiceState.voiceController || voiceState.voiceKeyActive) return;
...
export function handleVoiceHotkeyUp(
  voiceState: VoiceState,
  event: KeyboardEvent | null
): void {
  if (!voiceState.voiceKeyActive) return;
...
export function initTerminalVoice(
  voiceState: VoiceState,
  socket: WebSocket | null,
  deps: VoiceDeps
): void {
  voiceState.voiceBtn = document.getElementById("terminal-voice") as HTMLButtonElement | null;
  voiceState.voiceStatus = document.getElementById("terminal-voice-status") as HTMLElement | null;
  voiceState.mobileVoiceBtn = document.getElementById("terminal-mobile-voice") as HTMLButtonElement | null;
  voiceState.textVoiceBtn = document.getElementById("terminal-text-voice") as HTMLButtonElement | null;
  ...
  if (voiceState.voiceBtn && voiceState.voiceStatus) {
    initVoiceInput({
      ...
      .then((controller) => {
        if (!controller) {
          (voiceState.voiceBtn as HTMLElement)?.closest(".terminal-voice")?.classList.add("hidden");
          return;
        }
        ...
        const boundKeyDown = (e: KeyboardEvent) => handleVoiceHotkeyDown(voiceState, e, deps.getSocket());
        const boundKeyUp = (e: KeyboardEvent) => handleVoiceHotkeyUp(voiceState, e);
