...
from ...services.pma.managed_thread_automation import (
    cancel_unified_pma_subscription,
    cancel_unified_pma_timer,
    create_unified_pma_subscription,
    create_unified_pma_timer,
    subscription_request_has_explicit_routing,
    touch_unified_pma_timer,
    unified_pma_automation_read_model,
    unified_subscription_rows,
    unified_timer_rows,
)
...
from ...services.pma.managed_thread_read_models import (
    ...
    serialize_managed_thread_queue_item,
    serialize_managed_thread_turn_summary,
)
from ...services.pma.managed_thread_retirement import (
    retire_active_chat_index_targets,
)
...
def build_automation_routes(
    ...
    async def create_automation_subscription(
        ...
        try:
            ...
            if not subscription_request_has_explicit_routing(normalized_payload):
                normalized_payload = apply_origin_followup_context(
                    normalized_payload,
                    runtime_state,
                )
            created = create_unified_pma_subscription(request, normalized_payload)
    ...
    async def list_automation_subscriptions(
        ...
        return {
            "subscriptions": unified_subscription_rows(
                request,
                repo_id=repo_id,
                run_id=run_id,
                thread_id=thread_id,
    ...
    async def delete_automation_subscription(
        ...
        deleted = cancel_unified_pma_subscription(request, normalized_id)
        return {
            "status": "ok",
    ...
    async def create_automation_timer(
        ...
        try:
            normalized_payload = payload.normalized_payload()
            created = create_unified_pma_timer(request, normalized_payload)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    async def touch_automation_timer(
        ...
        try:
            ...
            touched = touch_unified_pma_timer(
    ...
    async def cancel_automation_timer(
        ...
        try:
            ...
            cancelled = cancel_unified_pma_timer(request, normalized_id)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        return {
            "status": "ok",
...
def build_managed_thread_crud_routes(
            ...
            try:
                if resolved.scope is not None:
                    thread = service.create_thread_target(
                        ...
                        thread_target_id=normalize_optional_text(
    ...
    def retire_active_managed_threads(request: Request) -> dict[str, Any]:
        ...
        context = get_pma_request_context(request)
        store = context.thread_store()
        retirement = retire_active_chat_index_targets(
            hub_root=context.hub_root,
            service=service,
            store=store,
        )
        ...
        return {
            "threads": [
                ...
                for thread in retirement["retired_threads"]
            ],
            "retired_surfaces": retirement["retired_surfaces"],
            "retired_count": retirement["retired_count"],
            "requested_count": retirement["requested_count"],
            "errors": retirement["errors"],
            "error_count": retirement["error_count"],
        }
