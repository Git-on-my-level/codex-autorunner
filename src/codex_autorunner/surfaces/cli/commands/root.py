...
def register_root_commands(app: typer.Typer) -> None:
    ...
    def init(
        ...
        git_init: bool = typer.Option(
            False, "--git-init", help="Run git init if missing"
        ),
        ...
        if target_root is None:
            ...
            if mode in ("hub",) or (mode == "auto" and _has_nested_git(target_root)):
                ...
                try:
                    ...
                    _raise_exit(f"git init failed: {exc}", cause=exc)
    ...
    def sessions(
        ...
        for entry in sessions_payload:
            ...
            repo_path = (
                entry.get("abs_repo_path") or entry.get("repo_path") or "unknown"
            )
    ...
    def usage(
        ...
        codex_home: Optional[Path] = typer.Option(
            None,
            "--codex-home",
            help="Override CODEX_HOME (defaults to env or ~/.codex)",
        ),
        ...
        until: Optional[str] = typer.Option(
            None,
            "--until",
            help="Upper bound ISO timestamp filter",
        ),
        output_json: bool = typer.Option(False, "--json", help="Emit JSON output"),
    ):
