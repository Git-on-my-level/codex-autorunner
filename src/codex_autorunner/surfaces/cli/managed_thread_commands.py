...
from .hub_control_plane_client import (
    build_hub_control_plane_url as _build_hub_control_plane_url,
)
from .hub_control_plane_client import (
...
from .pma_tail_types import (
    ManagedThreadStatusSnapshot as _ManagedThreadStatusSnapshot,
)
from .pma_tail_types import (
...
def _hub_request(
    ...
    *,
    ...
    try:
        ...
        data = _request_json(
            method,
            _build_hub_control_plane_url(config, path_suffix),
            body,
...
def _render_thread_status_snapshot(
    ...
    *,
    ...
    snapshot = _ManagedThreadStatusSnapshot.from_dict(data)
...
def _resolve_latest_managed_turn_id(config, *, managed_thread_id: str) -> str:
    turns_data = _request_json(
        "GET",
        _build_hub_control_plane_url(config, f"/threads/{managed_thread_id}/turns"),
        token_env=config.server_auth_token_env,
        params={"limit": 1},
    )
...
def managed_thread_spawn(
    ...
    try:
        ...
        data = _request_json(
            "POST",
            _build_hub_control_plane_url(config, "/threads"),
            request_payload,
            token_env=config.server_auth_token_env,
        )
    except httpx.HTTPError as exc:
...
def managed_thread_send(
    ...
    try:
        ...
        status_code, data = _request_json_with_status(
            "POST",
            _build_hub_control_plane_url(
                config, f"/threads/{managed_thread_id}/messages"
            ),
        ...
        if watch:
            ...
            try:
                status_data = _request_json(
                    "GET",
                    _build_hub_control_plane_url(
                        config, f"/threads/{managed_thread_id}/status"
                    ),
                    token_env=config.server_auth_token_env,
                    params={"limit": 1},
                )
...
def managed_thread_cancel_queued(
    managed_thread_id: str = typer.Option(
        ..., "--id", help="Managed PMA thread id", show_default=False
    ),
...
def managed_thread_clear_queue(
    managed_thread_id: str = typer.Option(
        ..., "--id", help="Managed PMA thread id", show_default=False
    ),
    output_json: bool = typer.Option(False, "--json", help="Emit JSON output"),
    ...
    try:
        ...
        if not follow:
            data = _request_json(
                "GET",
                _build_hub_control_plane_url(
                    config, f"/threads/{managed_thread_id}/tail"
                ),
                token_env=config.server_auth_token_env,
                params=params,
            )
        ...
        url = _build_hub_control_plane_url(
            config, f"/threads/{managed_thread_id}/tail/events"
        )
        with httpx.stream(
            "GET",
            url,
        ...
        for target_id in target_ids:
            try:
                data = _request_json(
                    "POST",
                    _build_hub_control_plane_url(
                        config, f"/threads/{target_id}/compact"
                    ),
...
def managed_thread_resume(
    ...
    try:
        ...
        thread_data = _request_json(
            "GET",
            _build_hub_control_plane_url(config, f"/threads/{managed_thread_id}"),
            token_env=config.server_auth_token_env,
        )
        ...
        data = _request_json(
            "POST",
            _build_hub_control_plane_url(
                config, f"/threads/{managed_thread_id}/resume"
            ),
            {},
            token_env=config.server_auth_token_env,
        )
...
def managed_thread_interrupt(
    ...
    thread_url = _build_hub_control_plane_url(config, f"/threads/{managed_thread_id}")
    try:
        thread_data = _request_json(
    ...
    try:
        status_code, data = _request_json_with_status(
            "POST",
            _build_hub_control_plane_url(
                config, f"/threads/{managed_thread_id}/interrupt"
            ),
            token_env=config.server_auth_token_env,
        )
...
def register_thread_commands(app: typer.Typer) -> None:
    ...
    app.command("spawn")(managed_thread_spawn)
    app.command("create")(managed_thread_spawn)
    app.command("list")(managed_thread_list)
    app.command("info")(managed_thread_info)
    app.command("status")(managed_thread_status)
    app.command("send")(managed_thread_send)
    app.command("turns")(managed_thread_turns)
    app.command("queue")(managed_thread_queue)
    app.command("cancel-queued")(managed_thread_cancel_queued)
    app.command("clear-queue")(managed_thread_clear_queue)
    app.command("output")(managed_thread_output)
    app.command("subscribe")(managed_thread_subscribe)
    app.command("tail")(managed_thread_tail)
