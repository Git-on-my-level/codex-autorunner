...
DESTINATION_QUICKSTART_BASENAME = "DESTINATION_QUICKSTART.md"
DESTINATION_QUICKSTART_REL_PATH = (
    Path(".codex-autorunner") / DESTINATION_QUICKSTART_BASENAME
)
...
DESTINATION_QUICKSTART_GENERATED_MARKER = "<!-- CAR:DESTINATION_QUICKSTART -->"
...
CAR_CONTEXT_HINT = (
    ...
    "for docker runtime setup use .codex-autorunner/DESTINATION_QUICKSTART.md "
    "or `car hub destination set --help`."
)
...
def build_about_car_markdown(
    *,
    ...
    return (
        ...
        "- Lint ticket frontmatter after edits (runs against all tickets): `car tickets lint --repo <path>`.\n\n"
        "## Ticket flow quickstart\n"
        f"- Read `{_display_path(repo_root, TICKET_FLOW_QUICKSTART_REL_PATH)}` for CLI entrypoints + gotchas.\n\n"
        "## Runtime quickstart\n"
        f"- Read `{_display_path(repo_root, DESTINATION_QUICKSTART_REL_PATH)}` for local/docker destination setup.\n\n"
        ...
        "- After any ticket edits, lint all tickets with `car tickets lint --repo <path>`.\n\n"
        "## Ticket templates\n"
...
def build_ticket_flow_quickstart_markdown(*, repo_root: Path) -> str:
    ...
    destination_quickstart_path = _display_path(
        repo_root, DESTINATION_QUICKSTART_REL_PATH
    )
    return (
        ...
        f"- For profile/mount/env options, see `{destination_quickstart_path}`.\n\n"
        "## Where tickets live\n"
...
def build_destination_quickstart_markdown(*, repo_root: Path) -> str:
    destinations_doc = _display_path(
        repo_root, Path("docs/configuration/destinations.md")
    )
    schema_doc = _display_path(repo_root, Path("docs/reference/hub-manifest-schema.md"))
    return (
        f"{DESTINATION_QUICKSTART_GENERATED_MARKER}\n"
        ...
        f"- Runtime guide: `{destinations_doc}`\n"
        f"- Manifest contract: `{schema_doc}`\n\n"
        "## Notes\n"
...
def build_tickets_agents_markdown(*, repo_root: Path) -> str:
    ...
    return (
        ...
        "- After edits, lint tickets with `car tickets lint --repo <path>`.\n\n"
        "## Ticket CLI (portable)\n"
        ...
        "- Lint: `car tickets lint --repo <path>`\n\n"
        "## Ticket flow (runner)\n"
        f"- See `{quickstart_path}` for `car ticket-flow ...` commands.\n"
    )
...
def ensure_destination_quickstart_file_for_repo(
    repo_root: Path, *, force: bool = False
) -> Path:
    path = repo_root / DESTINATION_QUICKSTART_REL_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
    content = build_destination_quickstart_markdown(repo_root=repo_root)
    if content and not content.endswith("\n"):
        content += "\n"
    ...
    if path.exists() and not force:
        try:
            ...
            existing = ""
        if DESTINATION_QUICKSTART_GENERATED_MARKER not in existing:
            return path
        if existing == content:
            return path
