...
- **Adapters**: `src/codex_autorunner/adapters/` (GitHub, Telegram, App Server).
  - PMA chat delivery follows the same split: `core/chat_delivery.py` emits transport-agnostic attempts, `chat_delivery_runtime.py` (at the package root `src/codex_autorunner/`) dispatches them, and Discord/Telegram adapters own formatting, topic parsing, and outbox writes. Duplicate suppression and noop detection are domain-owned via `pma_domain/publish_policy.py`.
...
- **Surfaces**:
  ...
  - **Web/UI**: `src/codex_autorunner/surfaces/web/` (FastAPI routes, web services), `src/codex_autorunner/web_frontend/` (Svelte source), and ignored package artifacts under `src/codex_autorunner/web_static/`.
