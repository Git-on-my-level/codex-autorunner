...
## Identity
CAR is a local-first, filesystem-backed agent orchestration system with multiple user surfaces (Discord, Telegram, Web, CLI) and multiple execution backends (Codex, OpenCode, OMP, Hermes, and other ACP-backed runtimes). It optimizes for leverage, speed, and evolvability.

The canonical list of surfaces, backends, and adapters is maintained here; other docs reference this section rather than duplicating it.
