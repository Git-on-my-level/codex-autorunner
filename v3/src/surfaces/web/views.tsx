...
export function InboxPage(props: { rows: EventRow[]; hasMore: boolean; filters: InboxFiltersView; vendors: string[]; severities: string[]; states: string[]; }) {
  ...
  const filterForm = (prefix = "") => {
    ...
    return <form class="filter-bar" method="get" action={`${UI_ROOT}/events`} aria-label="Filter inbox">
      ...
      {activeFilters > 0 && <a class="button ghost" href={`${UI_ROOT}/events`}>Clear all</a>}
    </form>;
  };
  return <Layout title="Event inspector" active={`${UI_ROOT}/events`}>
    <PageHeader title="Event inspector" description="Attention and meaningful outcomes, newest first." />
...
export function LoginPage() { return <Layout title="Sign in"><div class="sign-in"><div class="page-header-copy" style="margin-bottom:18px"><div class="page-eyebrow">CAR v3</div><h1>Sign in</h1><p class="page-description">Use your workspace’s web token to view private decisions and record your answers.</p></div><div class="panel"><div class="panel-body"><form class="stack" method="post" action={`${UI_ROOT}/login`}><div class="field"><label for="web-token">Web token</label><input id="web-token" name="token" type="password" autocomplete="current-password" required /></div><button class="button primary" type="submit">Sign in</button></form></div></div></div></Layout>; }
