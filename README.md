...
3) Open `http://127.0.0.1:4173` for the hub UI; repo UIs live under `/repos/<repo_id>/`. FastAPI endpoints are under `/api/*` (repo) and `/hub/*` (hub).
   ...
   - Repo IDs are URL-safe slugs derived from repo display names (usually directory names). The manifest stores `display_name` separately; upgrading from manifest v2 to v3 rewrites unsafe IDs and may suffix collisions (`repo-2`, etc.).
